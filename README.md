...
Trong Vocabulary, mỗi lần bấm vào tab, mở trực tiếp `#vocabulary`, F5 tại tab này hoặc bấm **Next**, app sẽ chọn ngẫu nhiên một từ **Need review / Chưa nhớ** khác nếu còn lựa chọn. Từ **Remembered / Đã nhớ** không được đưa vào lượt ôn. Nếu toàn bộ từ đều đã nhớ, app mới dùng lại toàn bộ danh sách để ôn tự do. Nút **Reset all to Need review / Đặt tất cả về Chưa nhớ** đưa cả bộ từ về lượt ôn. Khung tiến độ được căn giữa theo khung flashcard. Collocation luôn hiển thị trên mặt trước. Mỗi từ trong bảng có nút **Edit / Sửa** để cập nhật từ, IPA, từ loại, nghĩa, collocation và ví dụ; thao tác này giữ nguyên ID, trạng thái ghi nhớ và số lượt ôn. Khi thêm từ, chỉ cần nhập **Word / phrase** rồi bấm **Auto-fill** (hoặc bấm **Add flashcard** ngay): app lấy IPA thật từ mục Pronunciation của Wiktionary, dùng Datamuse cho nghĩa/từ loại, và điền ví dụ/collocation từ dữ liệu TOEIC có sẵn khi có. Sau khi thêm, form tự xóa và vẫn mở để nhập liên tiếp; bấm **X** hoặc **Cancel** khi đã xong. App không hiển thị ARPAbet dưới nhãn IPA. Nếu không lấy được IPA thật, trường IPA được để trống hoặc dùng dữ liệu offline đã kiểm tra.
...
1. Trên máy đang có dữ liệu mới nhất, vào **Settings → Export for GitHub**. App tải file cố định tên `toeic-data.json`. Có thể bấm **Open GitHub upload / Mở trang upload GitHub** để mở thẳng thư mục `dist/data` của repository.
